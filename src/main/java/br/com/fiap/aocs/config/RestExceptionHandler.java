...
public class RestExceptionHandler {
    ...
    public ResponseEntity<RestValidationError> HttpMessageNotReadabledHandler(HttpMessageNotReadableException e) {// escre
        ...
        // e.getFieldErrors().forEach(r -> fieldErrors.add(new
        // RestValidationError(r.getField(),r.getDefaultMessage())));

        return ResponseEntity.badRequest().body(new RestValidationError(field, message));

    }

    // @ExceptionHandler(RestNotFoundException.class) // incluir o nome da exception.class
    // public ResponseEntity<RestValidationError> RestNotFoundHandler(RestNotFoundException e) {// escre

    //     log.error("Erro de validação");
    //     String field = "";
    //     String message = "";

    //     if (e instanceof RestNotFoundException) {
    //         InvalidFormatException causa = (InvalidFormatException) e.getCause();
    //         field = causa.getPath().get(0).getFieldName();
